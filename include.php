...
class CCustomTypeComplexProperty
{
    function GetUserTypeDescription() 
    {
        return array(
            ...
            'PrepareSettings' => array('CCustomTypeComplexProperty', 'PrepareSettings'),
            'GetSettingsHTML' => array('CCustomTypeComplexProperty', 'GetSettingsHTML'),
            'GetPropertyFieldHtml' => array('CCustomTypeComplexProperty', 'GetPropertyFieldHtml'),
            'GetPropertyFieldHtmlMulty' => array('CCustomTypeComplexProperty', 'GetPropertyFieldHtmlMulty'),
            'GetPublicEditHTML' => array('CCustomTypeComplexProperty', 'GetPublicEditHTML'),
            'ConvertToDB' => array('CCustomTypeComplexProperty', 'ConvertToDB'),
            'ConvertFromDB' => array('CCustomTypeComplexProperty', 'ConvertFromDB'),
            'GetPublicViewHTML' => array('CCustomTypeComplexProperty', 'GetPublicViewHTML'),
            'GetSearchContent' => array('CCustomTypeComplexProperty', 'GetSearchContent'),
            'GetAdminFilterHTML' => array('CCustomTypeComplexProperty', 'GetAdminFilterHTML'),
            'GetAdminListViewHTML' => array('CCustomTypeComplexProperty', 'GetAdminListViewHTML'),
            'GetPublicFilterHTML' => array('CCustomTypeComplexProperty', 'GetPublicFilterHTML'),
        );
    }
    ...
    function GetAllProps($IBLOCK_ID, $PID)
    {
        ...
        while ($arr = $res->Fetch())
        {
            ...
            if (!in_array($arr["USER_TYPE"], $GLOBALS["COMPLEX_PROP_FORBIDDEN_UT"]))
                $props_[$arr["ID"]] = $arr;
        }
    ...
    function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName) 
    {
        ...
        if (is_array($arProperty["USER_TYPE_SETTINGS"]["SUBPROPS"]) && is_array($PROP) && count($PROP) > 0)
        {
            ...
            foreach ($arProperty["USER_TYPE_SETTINGS"]["SUBPROPS"] as $sp_num => $sprop)
            {
                ...
                {
                    if ($prop_fields_["ID"] == $sprop && $prop_fields_["MULTIPLE"] != "Y" && !in_array($prop_fields_["USER_TYPE"], $GLOBALS["COMPLEX_PROP_FORBIDDEN_UT"]))
                    {
    ...
    function GetPropertyFieldHtmlMulty($arProperty, $value, $strHTMLControlName) 
    {
        ...
        foreach ($value as $value_id => $value_arr)
        {
            ...
            if (is_array($arProperty["USER_TYPE_SETTINGS"]["SUBPROPS"]) && is_array($PROP) && count($PROP) > 0)
            {
                if ($hides[$value_id])
                {
                    echo '<div id="complex_property_add_area_'.IntVal($arProperty["ID"]).'_'.htmlspecialcharsbx($value_id).'" style="display:none">';
                    
                    echo '<input type="hidden" id="complex_property_add_hidden_'.IntVal($arProperty["ID"]).'_'.htmlspecialcharsbx($value_id).'" name="no_'.IntVal($arProperty["ID"]).'_'.htmlspecialcharsbx($value_id).'" value="y">';
                }
        ...
        if (count($hides) > 0  && is_array($PROP) && count($PROP) > 0)
        {
            ...
            function ComplexPropertyAddValue'.IntVal($arProperty["ID"]).'(what)
            {
            ...
            $strResult .= ');
                ...
                {
                    var aid = hides_js[i];
                    var sc_area_id = "complex_property_add_area_'.IntVal($arProperty["ID"]).'_" + aid;
                    var sc_inp_id = "complex_property_add_hidden_'.IntVal($arProperty["ID"]).'_" + aid;
            ...
            <input type="button" name="apply" class="adm-btn" value="'.GetMessage('SMCP_ADD_CVAL').'" onclick="ComplexPropertyAddValue'.IntVal($arProperty["ID"]).'(this)">';
        }
            ...
            if($arE = $res->Fetch())
            {
                ...
                if (is_array($value["VALUE"]["SUB_VALUES"]))
                {
                    foreach ($value["VALUE"]["SUB_VALUES"] as $pid => $parr)
                    {
                        ...
                        $parr["MULTIPLE"] = "N";
                        $return["SUB_VALUES"][$pid] = CIBlockFormatProperties::GetDisplayValue($arE, $parr, "complex_property");
...
class CIBEditComplexProp
{
    function OnBeforePrologHandler()
    {
        ...
        $GLOBALS["COMPLEX_PROP_FORBIDDEN_UT"] = Array(
            "complex_property",
            "video",
            "map_yandex",
            "map_google",
